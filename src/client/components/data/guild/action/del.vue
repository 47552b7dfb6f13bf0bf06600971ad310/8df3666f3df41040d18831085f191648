<template>
  <UiContent title="Giải Tán" sub="Xác nhận xóa gia tộc" class="bg-card rounded-2xl p-4" no-dot>
    <template #more>
    ...
    <UAlert title="Chú Ý" icon="i-bxs-info-circle" color="red" variant="soft">
      <template #description>
        Bạn chắc chắn muốn giải tán gia tộc <b>{{ guild.name }}</b> của mình ?
      </template>
    </UAlert>
...
const action = async (_id) => {
  try {
    loading.value = true
    await useAPI('guild/public/action/del')
