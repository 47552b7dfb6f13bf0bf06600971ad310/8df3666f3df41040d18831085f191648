<template>
  <div>
    <UCard class="bg-gray" :ui="{ 
      ...
      <template #header>
        <UiFlex>
          ...
          <UForm @submit="getList" class="max-w-[9rem] mr-auto">
            <UInput v-model="page.search.key" placeholder="Tìm kiếm..." icon="i-bx-search" size="md"></UInput>
          </UForm>
          
          <SelectDate time v-model="page.range.start" placeholder="Bắt đầu" size="md" class="ml-1 max-w-[140px]"/>
          <SelectDate time v-model="page.range.end" placeholder="Kết thúc" size="md" class="ml-1 max-w-[140px]"/>
        </UiFlex>
      </template>
      ...
      <UTable v-model:sort="page.sort" :columns="columns" :rows="list">
        <template #code-data="{ row }">
          <UBadge color="primary" variant="soft" weight="semibold">{{ row.code }}</UBadge>
        </template>

        <template #coin-data="{ row }">
          <UBadge color="gray">{{ useMoney().toMoney(row.coin) }}</UBadge>
        </template>
    ...
    <UModal v-model="modal.undo" prevent-close>
      <UForm @submit="undoAction" class="bg-card rounded-2xl p-4">
