  ...
  <UiContent title="Notification" sub="Thông báo nền tảng">
    ...
    <UCard :ui="{ body: { padding: 'p-0 sm:p-0' } }">
      ...
      >
        ...
        <template #collab-data="{ row }">
          <UiText v-if="!!row.collab">[{{ row.collab.code }}] {{ row.collab.link }}</UiText>
          <UiText v-else>Tất cả</UiText>
        </template>
    ...
    <UModal v-model="modal.add" preventClose :ui="{width: 'sm:max-w-[900px]'}">
      <UForm :state="stateAdd" @submit="addAction" class="bg-card rounded-2xl p-4">
        <UFormGroup label="Cộng tác viên">
          <SelectCollab v-model="stateAdd.collab"></SelectCollab>
        </UFormGroup>
    ...
    <UModal v-model="modal.edit" preventClose :ui="{width: 'sm:max-w-[900px]'}">
      <UForm :state="stateEdit" @submit="editAction" class="bg-card rounded-2xl p-4">
        <UFormGroup label="Cộng tác viên">
          <SelectCollab v-model="stateEdit.collab"></SelectCollab>
        </UFormGroup>
...
const actions = (row) => [
  [{
    ...
    click: () => {
      ...
      stateEdit.value.collab = row.collab ? row.collab._id : null
