<template>
  <UiContent title="ECoin Shop History" sub="Danh sách lệnh giao dịch mua hiện vật ECoin">
    <UiFlex class="mb-4 gap-1">
      ...
      <UForm :state="page" @submit="page.current = 1, getList()">
        <UiFlex class="gap-1">
          ...
          <USelectMenu v-model="page.search.by" :options="['CODE', 'USER', 'SHOP']" />
        </UiFlex>
      </UForm>
    </UiFlex>
    ...
    <UCard :ui="{ body: { padding: 'p-0 sm:p-0' } }">
      ...
      >
        ...
        <template #user-data="{ row }">
          ...
          <ManageUser :user="row.user" v-else />
        </template>
    ...
    <UModal v-model="modal.success" preventClose>
      <UForm :state="stateSuccess" @submit="successAction" class="bg-card rounded-2xl p-4">
        ...
        <UFormGroup label="Số tiền">
          <UInput :model-value="useMoney().toMoney(stateSuccess.money)" readonly />
        </UFormGroup>
...
const successAction = async () => {
  try {
    ...
    await useAPI('ecoin/manage/shop/history/verify', JSON.parse(JSON.stringify(stateSuccess.value)))
...
const refuseAction = async () => {
  try {
    ...
    await useAPI('ecoin/manage/shop/history/verify', JSON.parse(JSON.stringify(stateRefuse.value)))
...
const getList = async () => {
  try {
    ...
    const data = await useAPI('ecoin/manage/shop/history/list', JSON.parse(JSON.stringify(page.value)))
