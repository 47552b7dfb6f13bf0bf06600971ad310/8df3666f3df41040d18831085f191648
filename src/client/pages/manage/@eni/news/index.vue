<template>
  <UiContent title="News" sub="Quản lý các tin tức">
    ...
    <UCard :ui="{ body: { padding: 'p-0 sm:p-0' } }">
      ...
      >
        ...
        <template #title-data="{ row }">
          <NuxtLink :to="`/news/${row.key}`" target="_blank" class="text-primary font-semibold min-w-[200px] max-w-[200px] block whitespace-normal">{{ row.title }}</NuxtLink>
        </template>
    ...
    <!-- Modal Add -->
    <UModal v-model="modal.add" preventClose :ui="{width: 'sm:max-w-[calc(90%)] md:max-w-[calc(80%)] lg:max-w-4xl'}">
