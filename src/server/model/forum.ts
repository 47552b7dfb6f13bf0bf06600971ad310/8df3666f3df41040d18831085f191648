...
export const DBForumCategorySub = (mongoose : Mongoose) => {
  const schema = new mongoose.Schema<IDBForumCategorySub>({ 
    category: { type: mongoose.Schema.Types.ObjectId, ref: 'ForumCategory', index: true },
    name: { type: String },
    key: { type: String },
    description: { type: String },
    display: { type: Boolean, default: true }
...
export const DBForumPost = (mongoose : Mongoose) => {
  const schema = new mongoose.Schema<IDBForumPost>({ 
    category: { type: mongoose.Schema.Types.ObjectId, ref: 'ForumCategory', index: true },
    sub: { type: mongoose.Schema.Types.ObjectId, ref: 'ForumCategorySub', index: true },
    creater: { type: mongoose.Schema.Types.ObjectId, ref: 'User', index: true },
...
export const DBForumPostLike = (mongoose : Mongoose) => {
  const schema = new mongoose.Schema<IDBForumPostLike>({ 
    post: { type: mongoose.Schema.Types.ObjectId, ref: 'ForumPost', index: true },
