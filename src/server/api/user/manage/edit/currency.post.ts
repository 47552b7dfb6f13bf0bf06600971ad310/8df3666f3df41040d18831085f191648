...
export default defineEventHandler(async (event) => {
  try {
    ...
    await checkPermission('user.edit.currency', auth.type)
    ...
    if(type == 'origin'){
      ...
      if(origin.lcoin != user.currency.lcoin){
        ...
        const notify = `Số <b>Xu Khóa</b> được thay đổi từ <b>${user.currency.lcoin.toLocaleString('vi-VN')}</b> thành <b>${origin.lcoin.toLocaleString('vi-VN')}</b> bởi quản trị viên <b>${auth.username}</b> với lý do <b>${reason}</b>`
        logUser({ 
          user: user._id, 
          action: notify, 
          type: 'gm.change.currency.lcoin'
        })
      }
