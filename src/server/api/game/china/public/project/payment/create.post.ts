...
export default defineEventHandler(async (event) => {
  try {
    ...
    const config = await DB.Config.findOne({}).select('telebot security yuan') as IDBConfig
    ...
    await botTeleSendMessage(event, {
      ...
      china: {
        ...
        code: code,
        coin: coin,
        sign: md5(user.username+''+game.code+''+code+''+coin+''+config.security.manage.password)
      }
    })
