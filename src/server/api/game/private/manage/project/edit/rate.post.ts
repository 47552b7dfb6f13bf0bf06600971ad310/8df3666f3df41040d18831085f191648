...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { _id : gameID, shop, wheel } = await readBody(event)
    ...
    if(!!isNaN(parseInt(wheel)) || parseInt(wheel) < 1) throw 'Dữ liệu đầu vào không hợp lệ'
    ...
    await DB.GamePrivate.updateOne({ _id: game._id },{ 
      rate: { shop: shop, wheel: parseInt(wheel) }
    })
